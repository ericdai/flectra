...
class AccountFollowupSendingResults(models.TransientModel):
    ...
    _description = 'Results from the sending of the different letters' \
                   ' and emails'
    ...
    def _get_description(self):
        self.description = self.env.context.get('description')

    @api.multi
    def _get_need_printing(self):
        self.needprinting = self.env.context.get('needprinting')

    description = fields.Text(
        "Description", readonly=True, compute='_get_description',
        default=lambda self: self.env.context.get('description'))
    needprinting = fields.Boolean(
        "Needs Printing", compute='_get_need_printing',
        default=lambda self: self.env.context.get('needprinting'))
...
class AccountFollowupPrint(models.TransientModel):
    ...
    date = fields.Date('Follow-up Sending Date', required=True,
                       ...
                       help="This field allow you to select a "
                            "forecast date to plan your follow-ups")
    followup_id = fields.Many2one('account_followup.followup', 'Follow-Up',
                                  default=_get_followup, required=True,
                                  readonly=True)
    ...
    partner_lang = fields.Boolean('Send Email in Partner Language',
                                  ...
                                  help='Do not change message text, if you '
                                       'want to send email in partner '
                                       'language, or configure from company')
    email_body = fields.Text('Email Body', default="")
    summary = fields.Text('Summary', readonly=True)
    test_print = fields.Boolean('Test Print',
                                help='Check if you want to print follow-ups '
    ...
    def process_partners(self, partner_ids, data):
        ...
        for partner in self.env['account_followup.stat.by.partner'].browse(
                partner_ids):
            name = partner.partner_id.latest_followup_level_id_without_lit.name
            if partner.max_followup_id.manual_action:
                partner_obj._partner_manual_action([partner.partner_id.id])
                nbmanuals = nbmanuals + 1
                key = \
                    partner.partner_id.payment_responsible_id.name or \
                    _("Anybody")
        ...
        action = partner_obj.browse(
            partner_ids_to_print).do_partner_print(data)
    ...
    def clear_manual_actions(self, partner_list):
        ...
        self.env['res.partner'].browse(partners_to_clear).action_done()
    ...
    def do_process(self):
        ...
        if nbactionscleared > 0:
            restot['resulttext'] = restot['resulttext'] + "<li>" + \
                                   _("%s partners have no credits and "
                                     "as such the action is cleared") % (
    ...
    def _get_partners_followp(self):
        ...
        # it is not necessary to change follow-up level of
        # account move lines without debit
        ...
        # Fill dictionary of accountmovelines to_update with the
        # partners that need to be updated
